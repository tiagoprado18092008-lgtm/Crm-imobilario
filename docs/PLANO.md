...
> **Estado: Fases 0–3, 5, 7, 8 e 9 concluídas. Fases 4 e 6 parciais (bloqueadas em contas externas: Zadarma e DocuSign). A seguir: Fase 10 (Automações).**
...
| **8 — Receita** ✅ | Avenças, MRR decomposto, faturas, cobranças, saúde do cliente, export CSV | ✅ `Subscription`/`Invoice`/`RevenueSnapshot`/`ClientHealth`; ✅ MRR decomposto que reconcilia; ✅ aging em 5 baldes; ✅ saúde do cliente com motivos escritos; ✅ export CSV; ✅ migração aplicada e verificada; 32 testes | **concluída** |
| **9 — Gestão** ✅ | Metas, leaderboard, relatórios com drill-down, permissões por papel | ✅ `Goal` com progresso vs. tempo decorrido; ✅ leaderboard ordenado por reuniões; ✅ **teste automatizado de que um BDR não vê negócios de outro**; ✅ teste de que todos os routers exigem sessão; 35 testes | **concluída** |
