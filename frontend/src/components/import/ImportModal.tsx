      ...
      <div style={{
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20 }}>
              {columns.map(c => (
                <span key={c} style={{
                  ...
                  background: c.includes('*') ? 'rgba(99,102,241,0.12)' : 'var(--hover-bg)',
                  color: c.includes('*') ? 'var(--accent)' : 'var(--text-secondary)',
                  border: `1px solid ${c.includes('*') ? 'rgba(99,102,241,0.3)' : 'var(--border-color)'}`,
                }}>
                  {c}
                </span>
              ))}
        ...
        {step === 'preview' && (
          <div>
            ...
            {/* Preview table */}
            <div style={{ overflowX: 'auto', marginBottom: 20, border: '1px solid var(--border-color)', borderRadius: 8 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead>
                  <tr style={{ background: 'var(--hover-bg)' }}>
                    {headers.map((h, i) => (
                      <th key={i} style={{ padding: '8px 10px', textAlign: 'left', color: 'var(--text-muted)', fontWeight: 600, whiteSpace: 'nowrap', borderBottom: '1px solid var(--border-color)' }}>
                        {h}
                      </th>
                    ))}
                  </tr>
